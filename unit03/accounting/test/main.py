...
class AccountingApp:
    ...
    def create_login_screen(self):
        ...
        tk.Label(login_frame, text="再次輸入密碼(註冊用)：", bg="white").grid(row=2, column=0, padx=5, pady=5)
        self.register_entry = tk.Entry(login_frame, show="*")
        self.register_entry.grid(row=2, column=1, padx=5, pady=5)
...
# add here
    def register(self):
        username = self.username_entry.get()
        password = self.password_entry.get()
        register_password = self.register_entry.get()

        if not username or not password or not register_password:
            messagebox.showerror("錯誤", "請填寫所有必填欄位（使用者名稱、密碼）")
            return
    ...
    def create_main_screen(self):
        for widget in self.master.winfo_children():
            widget.destroy()
        self.master.title("個人記帳程式："+self.current_user)
        title_frame = tk.Frame(self.master, bg="blue", height=50)
        title_frame.pack(fill=tk.X)
        title_label = tk.Label(title_frame, text="個人記帳程式", font=("微軟正黑體", 20, "bold"), bg="blue", fg="white")
        title_label.pack(expand=True)
